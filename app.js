...
const APPS_SCRIPT_API_ENDPOINT = "https://budget-api-166126275494.asia-northeast3.run.app"; // Node.js 서버 엔드포인트
const API_KEY = "LovelyNaonDaon"; // Node.js 서버 API 키
...
async function updateCalendarDisplay () {
  ...
  try {
    ...
    const finalTx = Array.isArray(latest) ? latest : [];
    ...
    // 변경 시에만 갱신
    if (!renderedFromCache || JSON.stringify(cachedArr) !== JSON.stringify(finalTx)) {
