...
/**
 * 검색창 옆 주기 선택 드롭다운 메뉴 2개를 채우고 기본값을 설정하는 함수 (개선된 버전)
 */
function populateCycleDropdowns() {
    const cycles = [];
    const firstCycleDate = new Date(2025, 0, 1); // 앱 시작 주기: 2025년 1월

    // 1. 기준이 될 현재 날짜의 '주기'를 명확하게 계산합니다.
    const today = new Date();
    const cycleDate = new Date(today);
    if (today.getDate() < 18) {
        // 18일 이전이면 이전 달 주기에 해당
        cycleDate.setMonth(cycleDate.getMonth() - 1);
    }

    // 2. 현재 주기부터 2025년 1월까지 한 달씩 빼면서 목록을 생성합니다.
    while (cycleDate >= firstCycleDate) {
        const year = cycleDate.getFullYear();
        const month = String(cycleDate.getMonth() + 1).padStart(2, '0');
        cycles.push(`${year}-${month}`);
        
        // 다음 루프를 위해 한 달 전으로 설정
        cycleDate.setMonth(cycleDate.getMonth() - 1);
    }
    ...
    // 4. 기본값을 설정합니다.
    searchEndCycleSelect.value = cycles[0]; // 가장 최신 주기
    // 시작 주기는 1년 전 또는 가능한 가장 오래된 주기로 설정
